...
Because S3 classes obey the semantics of copy-on-modify. Therefore every time you deposites from the bank account, you would get a new copy of the object.
...
Because S3 classes are not suitable for modelling state that changes over time. S3 methods should (almost) always return the same result when called with the same inputs.
...
Subclasses can access private methods from their superclasses, but not private fields or attributes.  
